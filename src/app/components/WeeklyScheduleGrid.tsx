...
const getCourseColor = (code: string) => {
  const index =
    Math.abs(code.split("").reduce((sum, c) => sum + c.charCodeAt(0), 0)) %
    courseColors.length;
...
export default function WeeklyScheduleGrid({
  ...
  const getSectionCombinations = (course: Course) => {
    ...
    if (!(lectures || labs || discussions)) {
      throw new Error(
        "No sections for this course! This is a database problem."
      );
    }
  ...
  useEffect(() => {
    ...
    const combineSchedules = (
      ...
      combinations.forEach((combo) => {
        const scheduledLecture = combo.lecture
          ? { ...parseCourseTimes(course, combo.lecture), type: "LEC" as const }
          : null;
        const scheduledLab = combo.lab
          ? { ...parseCourseTimes(course, combo.lab), type: "LBN" as const }
          : null;
        const scheduledDiscussion = combo.discussion
          ? {
              ...parseCourseTimes(course, combo.discussion),
              type: "DIS" as const,
            }
          : null;
  ...
  return (
    <div className="flex items-start gap-2 w-full">
      ...
      <div className="flex-1 min-w-[300px] ">
        <CourseList courses={currentSchedule?.courses || []}></CourseList>
      </div>
      {/* Grid */}
      <div
      ...
      >
        ...
        {days.map((day) => (
          <div
            key={day}
            className="border-l border-b border-gray-300 text-center font-bold bg-gray-100"
          >
            {day}
          </div>
        ))}
          ...
          return (
            ...
            <React.Fragment key={hour}>
              ...
              {days.map((day) => (
                <div
                  key={`${day}-${hour}`}
                  className="border-l border-b border-gray-300 relative min-h-10"
                >
                  ...
                  {scheduleOptions[selectedScheduleIndex]?.map((course) =>
                    course.times
                      .filter(
                        (t) =>
                          t.day === day &&
                          Math.floor(t.start) === hourToDecimal(hour)
                      )
                      .map((t, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`absolute left-0 right-0 px-1 text-xs font-bold rounded ${
                              course.conflict ? "bg-red-400" : course.color
                            }`}
